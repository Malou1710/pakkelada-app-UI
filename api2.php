...
/*
password: ["skal udfyldes og være 4700"];
nameSearch: ["valgfri"];
*/
...
if($data["password"]== "4700"){

    $sql= "SELECT * FROM pakkeafhentning WHERE 1=1 ";
    $bind=[];


    if (!empty($data["nameSearch"])) {
        $sql ="AND pakkeNummer =:pakkeNummer";
        $bind[":pakkeNummer"] = $data["nameSearch"];
    }

    $pakkeafhentning = $db->sql($sql, $bind);
    header("HTTP/1.1 200 ok");
    echo json_encode($pakkeafhentning);
